...
const RefresherContent = class {
    ...
    componentWillLoad() {
        if (this.pullingIcon === undefined) {
            this.pullingIcon = config.config.get('refreshingIcon', 'arrow-down');
        }
        if (this.refreshingSpinner === undefined) {
            const mode = core.getIonMode(this);
            this.refreshingSpinner = config.config.get('refreshingSpinner', config.config.get('spinner', mode === 'ios' ? 'lines' : 'crescent'));
        }
    }
    render() {
