...
export class OnetimepasswordPage implements OnInit {
  ...
  @Input("save_password") save_country;
  @Input("save_password") save_state;
  ...
  setUser() {
    ...
    if (this.page_type_reg === 'yes'  && this.page_type_fgt === 'no' && this.OTP === this.otp) {
        this.httpcalls.register(this.save_email, this.save_password, this.save_name, this.save_country, this.save_state);
        this.OTP = undefined;
        this.closeModal();
    }
